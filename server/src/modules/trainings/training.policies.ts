 ...
 * For routes scoped under /clubs/:clubId/trainings/:id
...
/**
 * requireTrainingOwnershipOrSuperAdmin
 * For routes scoped under /clubs/:clubId/trainings/:id
 * Verifies the training belongs to the club AND the user has rights.
 */
export async function requireTrainingOwnershipOrSuperAdmin(
  ...
  try {
    ...
    if (!clubId || !trainingId) {
      return next(ApiError.badRequest("Club and training identifiers are required"));
    }
    ...
    if (training.club.toString() !== clubId) {
      return next(ApiError.forbidden("Training does not belong to this club"));
    }
