...
function getLeaderId(c: any): string | undefined {
  if (!c.leader) return undefined;
  if (typeof c.leader === "string") return c.leader;
  return c.leader._id;
}
      ...
      <div className={styles.gaugeArea}>
        <div className={styles.gaugeWrap}>
          <svg viewBox="0 0 200 110" className={styles.gaugeSvg}>
            ...
            <path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke={balance >= 0 ? "#059669" : "#DC2626"} strokeWidth={14} strokeLinecap="round" strokeDasharray={`${circumference}`} strokeDashoffset={`${circumference - filled}`} />
          </svg>
...
export function ClubLeaderDashboard() {
  ...
  const { data: clubsData, isLoading: clubsLoading, error: clubsError } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data.filter(
        (c) => getLeaderId(c) === user?.id || user?.role === "super_admin"
      );
    },
    enabled: !!user,
  });
      ...
      <div className={styles.page}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 0" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ width: 40, height: 40, border: "3px solid #E5E7EB", borderTopColor: "#059669", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
            <p style={{ fontSize: 14, color: "#6B7280" }}>Chargement du tableau de bord...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (clubsError || !leaderClub || dashboardError || !data) {
    const errMsg = (dashboardError as any)?.error?.message || (dashboardError as any)?.message || "Aucun club assigné. Contactez l'administrateur.";
    return (
      <div className={styles.page}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 0" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ width: 56, height: 56, borderRadius: 16, background: "#FEF2F2", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#DC2626" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
            </div>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: "#111827", marginBottom: 4 }}>Erreur de chargement</h3>
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 16 }}>{errMsg}</p>
            <button onClick={() => window.location.reload()} style={{ padding: "8px 20px", background: "#059669", color: "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Réessayer</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Club : {leaderClub.name}</p>
        </div>
      ...
      <div className={styles.row3}>
        ...
        <a href="#notifications" className={styles.actionCard}>
